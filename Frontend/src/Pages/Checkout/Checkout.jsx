...
function Checkout() {
    const { language, user_id, windowWidth,products, offers, shippingPrices, shippingname, shipping, setShipping, setShippingName, shippingid, setShippingid } = useContext(MyContext);
    ...
    useEffect(() => {
        ...
        axios.get("https://dash.watchizereg.com/api/show_cart", {
            headers: { "Api-Code": apiCode }
        })
        .then(response => {
            if (response.data && Array.isArray(response.data)) {
                // Find the cart for this user
                const foundCart = response.data.find(cart => parseInt(cart.user_id) === parseInt(user_id));
                if (foundCart && Array.isArray(foundCart.cart_item)) {
                    setUserCart(foundCart.cart_item);
                } else {
                    setUserCart([]);
                }
            } else {
    ...
    // 1. Add this handler function inside your Checkout component:
const handleDeleteCartItem = async (itemId) => {
    try {
        await axios.delete(`https://dash.watchizereg.com/api/delete_cart/${itemId}`, {
            headers: { "Api-Code": apiCode }
        });
        ...
        showAlert(language === "ar" ? "فشل حذف المنتج" : "Failed to delete item", "error");
    }
    };
...
<FormControl component="fieldset" style={{ marginTop: 20, width: "100%" }}>
    <FormLabel component="legend" style={{ fontWeight: "bold", fontSize: "1.1rem", marginBottom: 8 }}>
    ...
    >
        <FormControlLabel
            value="cash"
            control={<Radio sx={{ color: "#262626", '&.Mui-checked': { color: "#262626" } }} />}
            label={<span style={{ fontWeight: 600, fontSize: "1rem" }}>{getLabel('cash')}</span>}
            sx={{ marginBottom: { xs: 1, sm: 0 } }}
        />
        <FormControlLabel
            value="card"
            control={<Radio sx={{ color: "#262626", '&.Mui-checked': { color: "#262626" } }} />}
            label={<span style={{ fontWeight: 600, fontSize: "1rem" }}>{getLabel('card')}</span>}
        />
    </RadioGroup>
</FormControl>
        ...
        >
            <div className="d-flex flex-column col-7">
                ...
                <span className="text-secondary" style={{ fontSize: "0.9rem" }}>
                    {item.quantity} × {(item.piece_price || 0)} {language === "ar" ? "ج.م" : "EGP"}
                </span>
            </div>
            <div className="col-3 text-end">
                <span className="fw-bold text-danger" style={{ fontSize: "1.1rem" }}>
                    {(item.piece_price || 0) * (item.quantity || 0)} {language === "ar" ? "ج.م" : "EGP"}
                </span>
            </div>
