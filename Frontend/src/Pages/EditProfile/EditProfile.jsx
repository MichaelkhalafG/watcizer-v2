...
function EditProfile() {
  ...
  const [phoneNumber, setPhoneNumber] = useState(
    sessionStorage.getItem('phone_number') !== 'null' ? sessionStorage.getItem('phone_number') : '',
  )
  ...
  const [image, setImage] = useState(
    sessionStorage.getItem('image') !== 'null' ? sessionStorage.getItem('image') : '',
  )
  ...
  const handleUpdateProfile = () => {
    ...
    http
      ...
      .then(async () => {
        ...
        if (image) {
          try {
            const response = await http.get('/all_user')
            let user_image = response.data.find((user) => user.id === user_id)?.image
            // console.log("User Image:", user_image);
            sessionStorage.setItem(
              'image',
              user_image
                ? `${import.meta.env.VITE_ASSET_BASE}/Uploads_Images/User/${user_image}`
                : null,
            )
  ...
  return (
    <div
      className={`container mt-md-4 mb-md-0 mb-5 mt-0 ${language === 'ar' ? 'text-right' : 'text-left'}`}
