...
function SearchPageForPhone() {
  ...
  const handleAddToCart = (product, type_stock) => {
    ...
    if (isNaN(totalPrice) || totalPrice <= 0) {
      setAlertMessage(
        language === 'ar' ? 'حدث خطأ في حساب السعر.' : 'Error calculating total price.',
      )
      setAlertType('error')
      setOpenAlert(true)
      return
  ...
  return (
    <div
      ref={containerRef}
      className="search-overlay position-fixed bg-white d-flex flex-column p-3"
    ...
    >
      ...
      <div
        className="search-box p-2 px-4 border rounded-3 d-flex align-items-center"
              ...
              <div className="card border-0 rounded-3 shadow-sm position-relative">
                ...
                <div className="card-body p-3">
                  ...
                  {user_id && user_id !== null ? (
                    <Link
                      onClick={() =>
                        handleAddToCart(product, parseInt(product.stock) > 0 ? 'Express' : 'Market')
                      }
