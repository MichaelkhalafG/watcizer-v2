...
function ListingGrades() {
    const { language, products, currentPage, tables,
        // offers, setCart, fetchCart, user_id,
        watches, fashion, setCurrentPage, gradesfilters, windowWidth, setgradesfilters, handleAddTowishlist } = useContext(MyContext);
    ...
    if (brands?.length) {
        filteredProducts = filteredProducts.filter((product) =>
            brands.includes(product.brand_id)
        );
    }
    if (subTypes?.length) {
        filteredProducts = filteredProducts.filter((product) =>
    ...
    if (grades?.length) {
        filteredProducts = filteredProducts.filter((product) =>
            grades.includes(product.grade_id)
        );
    }
