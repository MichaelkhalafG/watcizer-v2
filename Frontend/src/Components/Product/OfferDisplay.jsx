...
function OfferDisplay() {
  ...
  const fetchRatings = useCallback(async () => {
    try {
      const response = await http.get('/all_offer_rating')
  ...
  const handleRatingSubmit = async (value, comment) => {
    if (!user_id) {
      ...
      if (value && sanitizedComment.trim()) {
        try {
          await http.post('/add_offer_rating', null, {
            params: {
              offer_id: offer.id,
              rating: value,
      ...
      >
        <div className={`col-12 ${language === 'ar' ? 'text-end' : 'text-start'}`}>
          <h3 className="fw-bold">
            {language === 'ar' ? offer.offer_name_ar : offer.offer_name_en}
          </h3>
        </div>
        <div className="col-4">
          {renderDetail(
            ...
            {offer?.image && (
              <img
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  boxShadow:
                    offer.image === selectedImage ? '0px 4px 10px rgba(0, 0, 0, 0.2)' : 'none',
                }}
                className="thumbnail"
              />
            )}
