...
function ProductSlider({ text, gradeproducts, to, moreid }) {
    const { language, Loader,
        ...
        setCurrentPage, setgradesfilters, windowWidth,
        // offers, setCart,
        handleAddTowishlist } = useContext(MyContext);
    ...
    const handleAddToCart = useCallback((product, type_stock) => {
          const piecePrice = parseFloat(product.sale_price_after_discount || product.selling_price);
      
          if (isNaN(piecePrice) || piecePrice <= 0) {
            setAlertMessage(language === "ar" ? "حدث خطأ في حساب السعر." : "There was an error calculating the price.");
            setAlertType("error");
            setOpenAlert(true);
            return;
          }
    ...
    const ActionMenu = ({ product }) => (
        <div className="action-menu">
            {isMobile ? (
                <Link to={`/product/${product.product_title}`} className="btn btn-dark rounded-circle" title="display product">
                    <SlSizeFullscreen />
                </Link>
            ) : (
                <button className="btn btn-dark rounded-circle" onClick={() => handleProductClick(product)} title="display product">
                    <SlSizeFullscreen />
                </button>
            )}
            <button className="btn mt-2 btn-danger rounded-circle" onClick={() => handleAddTowishlist(product.id, "p")} title="add to wishlist">
            <FaRegHeart />
            </button>
        </div>
    );
                                    ...
                                    <div className="card product-card border-0 rounded-3 shadow-sm d-flex flex-column position-relative">
                                        ...
                                        <div className="card-body d-flex flex-column justify-content-between p-3">
                                            ...
                                            {/* {user_id && user_id !== null ? */}
                                                <button onClick={() => handleAddToCart(product, (parseInt(product.stock) > 0 ? 'Express' : "Market"))}
