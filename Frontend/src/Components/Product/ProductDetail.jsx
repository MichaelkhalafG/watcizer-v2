...
function ProductDetail() {
  ...
  useEffect(() => {
    if (item) {
      const n = getName(item, 'en')
      const b = getBrand(item, 'en')
      document.title = b ? `${n} | ${b} — Watchizer` : `${n} — Watchizer`
    }
    return () => {
  ...
  }, [userId, newReview, isOffer, offer, product, fetchRatings, showToast, isRTL, navigate])
    ...
    return (
      <div className="wz-pd">
        <div className="wz-pd-notfound">
          ...
          <button className="wz-pd-notfound-btn" onClick={() => navigate('/listing')}>
  ...
  // ── PDP SEO description: short description, HTML-stripped, capped at 160 chars ──
  const seoDesc = (
    (isRTL ? item?.short_description_ar : item?.short_description_en) ||
    item?.short_description ||
    item?.short_description_en ||
    `${name}${brandLabel ? ` – ${brandLabel}` : ''} — shop this timepiece at Watchizer.`
  )
    .toString()
    .replace(/<[^>]*>/g, ' ')
    .replace(/\s+/g, ' ')
  ...
  // ONE canonical pattern per product: /product/{english-slug} (offers: /offer/{slug}),
  // independent of which URL (/products/:id, raw-title, …) was used to reach here.
  const canonicalPath = isOffer ? offerUrl(item) : productUrl(item)
  const canonicalUrl = `${SEO_DOMAIN}${canonicalPath}`

  // 301-style canonical redirect: /products/:id and any non-canonical URL → the
  // slug URL. Client-side (replace) — a TRUE 301 requires host/SSR-level config.
  if (item && canonicalPath && location.pathname !== canonicalPath) {
    return <Navigate to={`${canonicalPath}${location.search}`} replace />
  }

  const priceNow = hasSale ? salePrice : price
  const ogTitle = `${name}${brandLabel ? ` – ${brandLabel}` : ''} | Watchizer`
  const ogImage = images[0] || `${SEO_DOMAIN}/logo.svg`
  // Middle breadcrumb mirrors the visual breadcrumb above (Offers / brand / all).
  const crumbMid = isOffer
    ? { name: isRTL ? 'العروض' : 'Offers', item: `${SEO_DOMAIN}/offers` }
    : brandLabel && brandHref
      ? { name: brandLabel, item: `${SEO_DOMAIN}${brandHref}` }
      : { name: isRTL ? 'كل المنتجات' : 'All Products', item: `${SEO_DOMAIN}/listing` }
  ...
  const productLd = {
    ...
    sku: String(isOffer ? (offer?.id ?? '') : (product?.id ?? '')),
    description: seoDesc,
    offers: {
      '@type': 'Offer',
      priceCurrency: 'EGP',
    ...
    // aggregateRating ONLY when real reviews exist (empty ratings are invalid).
    ...(reviewCount > 0
      ? {
          aggregateRating: {
  ...
  const breadcrumbLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: isRTL ? 'الرئيسية' : 'Home', item: `${SEO_DOMAIN}/` },
  ...
  return (
    <div className="wz-pd" dir={isRTL ? 'rtl' : 'ltr'}>
      <Helmet>
        <title>{`${name}${brandLabel ? ` – ${brandLabel}` : ''} | Watchizer`}</title>
        <meta name="description" content={seoDesc} />
        <link rel="canonical" href={canonicalUrl} />
        {/* Social share — per-product OG/Twitter (overrides the global defaults) */}
      ...
      <div className="wz-pd-inner">
        ...
        <nav className="wz-pd-bc" aria-label="Breadcrumb">
          <Link to="/" className="wz-pd-bc-link">
            {isRTL ? 'الرئيسية' : 'Home'}
          </Link>
          <span className="wz-pd-bc-sep">/</span>
          {isOffer ? (
            <Link to="/offers" className="wz-pd-bc-link">
              {isRTL ? 'العروض' : 'Offers'}
            </Link>
          ) : brandLabel && brandHref ? (
            <Link to={brandHref} className="wz-pd-bc-link">
              {brandLabel}
            </Link>
          ) : (
            <Link to="/listing" className="wz-pd-bc-link">
        ...
        <div className="wz-pd-grid">
          ...
          <div className="wz-pd-info">
            {brandLabel &&
              (brandHref ? (
                <Link to={brandHref} className="wz-pd-brand wz-pd-hide-mobile">
                  {brandLogo && (
                    <img
                      src={brandLogo}
                      alt=""
