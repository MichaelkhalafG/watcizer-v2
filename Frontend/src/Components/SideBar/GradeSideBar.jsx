...
function GradeSideBar({ setFilters }) {
    ...
    // If filters are empty, extract from URL and set them
    useEffect(() => {
    if (
        gradesfilters &&
        gradesfilters.categories.length === 0 &&
        gradesfilters.brands.length === 0 &&
        gradesfilters.subTypes.length === 0 &&
        gradesfilters.grades.length === 0
    ) {
        const gradeObj = tables?.grades?.find((gr) => gr.translations?.find((t)=> t.locale === "en").grade_name === grade);

        let updatedFilters = { ...gradesfilters, price: value };

        if (gradeObj) {
            updatedFilters.grades = [gradeObj.id];
        }

        setFilters(updatedFilters);
    }
}, [tables,gradesfilters, setFilters,grade, value]);
        ...
        switch (filterType) {
            ...
            case "grades":
                setSelectedgrades((prev) => {
                    const updated = updateSelectedItems(prev);
                    setFilters((filters) => ({ ...filters, grades: updated }));
                    return updated;
                });
                break;
            default:
    ...
    useEffect(() => {
        ...
        if (tables) {
            ...
            const localizedGrades = extractLocalizedNames(tables.grades || [], "grade_name");
            setCategories(localizedCategories);
            setBrands(localizedBrands);
            setSubTypes(localizedSubTypes);
            setgrades(localizedGrades);
        }
    }, [tables, language]);
                                    ...
                                    checked={
                                        filterType === "categories"
                                            ...
                                            : filterType === "brands"
                                                ...
                                                : filterType === "grades"
                                                    ? selectedgrades.includes(item.id)
        ...
        <div className="pt-2 container-fluid p-0" dir={direction}>
            <div className="tables-list-filter" style={{ height: "30vh", overflowY: "auto" }}>
                ...
                {[
                    ...
                    { name: isRTL ? "الحالة" : "Grades", data: grades, key: "grade_name", filterType: "grades" },
