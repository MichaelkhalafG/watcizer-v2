import { useSyncExternalStore } from 'react'
import { cartStore } from '../Store/cartStore'

// Single source of truth lives in cartStore.js — re-exported here so existing
// consumers can keep importing `getItemKey` from this hook.
export { getItemKey, getGuestToken } from '../Store/cartStore'

const useCart = () => {
  const cart = useSyncExternalStore(cartStore.subscribe, cartStore.getSnapshot)
  const extra = useSyncExternalStore(cartStore.subscribe, cartStore.getExtraSnapshot)

  return {
    cart,
    // existing API (unchanged)
    addItem: cartStore.addItem,
    updateQuantity: cartStore.updateQuantity,
    removeItem: cartStore.removeItem,
    clearCart: cartStore.clearCart,
    // new capabilities
    undoRemove: cartStore.undoRemove,
    validateCart: cartStore.validateCart,
    fetchCart: cartStore.fetchCart,
    mergeGuestCart: cartStore.mergeGuestCart,
    totals: extra.totals,
    warnings: extra.warnings,
    lastRemoved: extra.lastRemoved,
  }
}

export default useCart
