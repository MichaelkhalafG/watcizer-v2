...
(function () {
    ...
    const one = (selector, scope) => {
        ...
        return bypassSelector(base)
            ? Optional.none()
            : Optional.from(base.querySelector(selector)).map(
    ...
    const generateColumns = (rowData) => {
        ...
        each$2(rowData.cells, (column) => {
            ...
            range$1(colspan, (columnIndex) => {
                ...
                columnsGroup[colIndex] = columnext(
                    column.element,
                    colspan,
                    colIndex,
                );
    ...
    const generate$1 = (list) => {
        ...
        const tableOpt = head(list)
            ...
            .bind(table);
        const lockedColumns = tableOpt
            .bind(getLockedColumnsFromTable)
            .getOr({});
        let maxRows = 0;
        let maxColumns = 0;
        let rowCount = 0;
        ...
        each$2(rows, (rowData) => {
            ...
            each$2(rowData.cells, (rowCell) => {
                ...
                const isLocked = hasNonNullableKey(
                    lockedColumns,
                    start.toString(),
                );
    ...
    const isUnit = (unit, accepted) =>
        exists(accepted, (acc) =>
            exists(units[acc], (check) => unit === check),
        );
    ...
    const isPercentSizing$1 = (table) =>
        getRawWidth$1(table).exists((size) =>
            rPercentageBasedSizeRegex.test(size),
        );
    ...
    const getRawW = (cell) => {
        return getRawWidth$1(cell).getOrThunk(
            () => getPixelWidth$1(cell) + "px",
        );
    };
    const getRawH = (cell) => {
    ...
    const getWidthFrom = (warehouse, table, getWidth, fallback) => {
        ...
        const backups = [Optional.some(width.edge(table))].concat(
            map$1(width.positions(columnCells, table), (pos) =>
                pos.map((p) => p.x),
            ),
        );
    ...
    const getHeightFrom = (
        ...
        const backups = [Optional.some(direction.edge(table))].concat(
            map$1(direction.positions(rows$1, table), (pos) =>
                pos.map((p) => p.y),
            ),
        );
    ...
    const isTextNodeWithCursorPosition = (el) =>
        getOption(el)
            .filter(
                (text) => text.trim().length !== 0 || text.indexOf(nbsp) > -1,
            )
            .isSome();
    ...
    const isRectangular = (warehouse, bounds) => {
        ...
        for (let i = bounds.startRow; i <= bounds.finishRow; i++) {
            for (let j = bounds.startCol; j <= bounds.finishCol; j++) {
                isRect =
                    isRect &&
                    Warehouse.getAt(warehouse, i, j).exists(detailIsWithin);
            }
        }
    ...
    const retrieveBox = (
        ...
        return getEdges(
            ...
            const firstAncestor = ancestor$1(
                edges.first,
                sectionSelector,
                isRoot,
            );
            const lastAncestor = ancestor$1(
                edges.last,
                sectionSelector,
                isRoot,
            );
    ...
    const unmergable = (selectedCells) => {
        ...
        return selectedCells.length > 0 &&
            forall(selectedCells, hasRowOrColSpan)
    ...
    const strSelectedSelector =
        "td[" + strSelected + "],th[" + strSelected + "]";
    ...
    const resizeTable = () => {
        ...
        const calcRelativeDeltas = (sizes, index, delta, minCellSize) => {
            ...
            const newThis = Math.max(
                minCellSize,
                (sizes[index] + delta) / ratio,
            );
    ...
    const findCommonCellType = (cells) => {
        const headerCells = filter$2(cells, (cell) =>
            isHeaderCell(cell.element),
        );
    ...
    const generateSection = (table, sectionName) => {
        const section = child(table, sectionName).getOrThunk(() => {
            const newSection = SugarElement.fromTag(
                sectionName,
                owner(table).dom,
            );
    ...
    const toGrid = (warehouse, generators, isNew) => {
        ...
        each$2(warehouse.colgroups, (colgroup) => {
            ...
            for (
                ...
                const element = Warehouse.getColumnAt(warehouse, columnIndex)
                    ...
                    .getOrThunk(() =>
                        elementnew(generators.colGap(), true, false),
                    );
                colgroupCols.push(element);
            }
            grid.push(
                rowcells(colgroup.element, colgroupCols, "colgroup", isNew),
            );
        });
        for (let rowIndex = 0; rowIndex < warehouse.grid.rows; rowIndex++) {
            ...
            for (
                ...
                const element = Warehouse.getAt(
                    warehouse,
                    rowIndex,
                    columnIndex,
                )
                    .map((item) =>
                        elementnew(item.element, isNew, item.isLocked),
                    )
                    .getOrThunk(() =>
                        elementnew(generators.gap(), true, false),
                    );
        ...
        if (index > 0 && index < grid[0].cells.length) {
            each$2(grid, (row) => {
                ...
                while (
                    ...
                    comparator(
                        prevCell.element,
                        row.cells[index + offset].element,
                    )
                ) {
                    mutateCell(
                        row,
                        ...
                        elementnew(
                            substitute,
                            true,
                            row.cells[index + offset].isLocked,
                        ),
                    );
                    offset++;
                }
                        ...
                        if (isToReplace) {
                            ...
                            replacement.each((sub) => {
                                mutateCell(
                                    row,
                                    j,
                                    elementnew(sub, true, current.isLocked),
                                );
                            });
                        }
                    }
    ...
    const colFill = (grid, amount, generators, startIndex) =>
        map$1(grid, (row) => {
            const newChildren = generateElements(
                amount,
                row,
                generators,
                never,
    ...
    const lockedColFill = (grid, generators, lockedColumns) =>
        map$1(grid, (row) => {
            return foldl(
                lockedColumns,
                (acc, colNum) => {
                    const newChild = generateElements(
                        1,
                        row,
                        generators,
                        always,
    ...
    const insertRows = (index, gridA, gridB, generator, comparator) => {
        ...
        const { cols: oldCols, rows: oldRows } =
    ...
    const insertColumnAt = (grid, index, example, comparator, substitution) =>
        map$1(grid, (row) => {
            const withinSpan =
                ...
                comparator(
                    getCellElement(row, index - 1),
                    getCellElement(row, index),
                );
    ...
    const replaceRows = (
        ...
        const shouldReplaceCell = rowReplacerPredicate(
            targetRow,
            columnHeaders,
        );
    ...
    const determine = (input, column, step, tableSize, resize) => {
        ...
        const onOnly = (index) =>
            tableSize.singleColumnWidth(result[index], step);
    ...
    const adjustWidth = (table, delta, index, resizing, tableSize) => {
        ...
        resizing.resizeTable(
            tableSize.adjustTableWidth,
            clampedStep,
            isLastColumn,
        );
    };
    ...
    const opEraseColumns = (grid, extractDetail, _comparator, _genWrappers) => {
        ...
        const maxColIndex =
            newGrid.length > 0 ? newGrid[0].cells.length - 1 : 0;
        return bundle(
            newGrid,
            columns[0].row,
    ...
    const opGetColumnsType = (table, target) => {
        ...
        return details
            .bind((selectedCells) => {
                ...
                const maxColRange =
                    lastSelectedCell.column + lastSelectedCell.colspan;
                const selectedColumnCells = flatten(
                    map$1(house.all, (row) =>
                        filter$2(
                            row.cells,
                            (cell) =>
                                cell.column >= minColRange &&
                                cell.column < maxColRange,
                        ),
                    ),
                );
    ...
    const opGetRowsType = (table, target) => {
        ...
        return details
            .bind((selectedCells) => {
                ...
                const maxRowRange =
                    lastSelectedCell.row + lastSelectedCell.rowspan;
    ...
    const TableActions = (editor, resizeHandler, cellSelectionHandler) => {
        ...
        const colMutationOp = isResizeTableColumnResizing(editor)
            ? noop
            : halve;
        ...
        const execute =
            ...
            (table, target, noEvents = false) => {
                ...
                return guard(table)
                    ? operation(table, target, generators, behaviours).bind(
                          (result) => {
                              ...
                              const range = setSelectionFromAction(
                                  table,
                                  result,
                              );
                              if (inBody(table)) {
    ...
    const copyCols = (table, target) => {
        ...
        return details.map((selectedCells) => {
            ...
            const maxColRange =
                lastSelectedCell.column + lastSelectedCell.colspan;
            const fakeColGroups = generateColGroup(
                house,
                minColRange,
                maxColRange,
            );
    ...
    const sum = (values, fallback) => {
        ...
        return foldr(
            ...
            (rest, v) => {
                return (
                    Size.from(v).fold(constant(0), identity, identity) + rest
                );
            },
            0,
        );
    ...
    const redistribute = (table, optWidth, optHeight) => {
        ...
        optHeight.each((newHeight) => {
            ...
            const nuHeights = redistribute$1(
                oldHeights,
                totalHeight,
                newHeight,
            );
    ...
    const isPercentage = (width) =>
        isString(width) && width.indexOf("%") !== -1;
    ...
    const registerCommands = (editor, actions) => {
        ...
        editor.addCommand("mceTableApplyCellStyle", (_ui, args) => {
            ...
            const validArgs = filter$1(
                args,
                (value, style) =>
                    editor.formatter.has(getFormatName(style)) &&
                    isString(value),
            );
    ...
    const registerQueryCommands = (editor, actions) => {
        ...
        each$1(
            {
                mceTableRowType: () =>
                    lookupOnSelection(actions.getTableRowType),
                mceTableCellType: () =>
                    lookupOnSelection(actions.getTableCellType),
                mceTableColType: () =>
                    lookupOnSelection(actions.getTableColType),
            },
    ...
    const getRanges = (win, selection) =>
        selection.match({
            ...
            relative: (startSitu, finishSitu) => {
                return {
                    ltr: cached(() =>
                        relativeToNative(win, startSitu, finishSitu),
                    ),
                    rtl: cached(() =>
                        Optional.some(
    ...
    const doDiagnose = (win, ranges) => {
        ...
        if (rng.collapsed) {
            const reversed = ranges
                .rtl()
                .filter((rev) => rev.collapsed === false);
            return reversed
                .map((rev) =>
                    adt$3.rtl(
            ...
            return identify(start, finish, isRoot).bind((cellSel) => {
                ...
                if (boxes.length > 1) {
                    ...
                    return Optional.some(
                        Response.create(
                            Optional.some(
                                makeSitus(start, 0, start, getEnd(start)),
                            ),
                            true,
                        ),
                    );
            ...
            .bind((afterCell) => {
                return closest$1(before, "td,th", isRoot).map((beforeCell) => {
                    if (!eq$1(afterCell, beforeCell)) {
                        ...
                        return eq$1(after, afterCell) &&
                            getEnd(afterCell) === afterOffset
                            ? failure(beforeCell)
            ...
            return closest$1(startRow, "table", isRoot).bind((table) => {
                ...
                if (eq$1(startRow, rows[0])) {
                    return seekLeft(
                        ...
                        return Response.create(
                            Optional.some(
                                makeSitus(last, lastOffset, last, lastOffset),
                            ),
                            true,
                        );
                    });
                ...
                findCell(event.target, isRoot).each((finish) => {
                    identify(start, finish, isRoot).each((cellSel) => {
                        ...
                        if (boxes.length === 1) {
                            ...
                            const isNonEditableCell =
                                getRaw(singleCell) === "false";
    ...
    const caretPositionFromPoint = (doc, x, y) => {
        var _a, _b;
        return Optional.from(
            (_b = (_a = doc.dom).caretPositionFromPoint) === null ||
                _b === void 0
                ? void 0
                : _b.call(_a, x, y),
        ).bind((pos) => {
    ...
    const setToElement = (win, element, selectNodeContents$1 = true) => {
        const rngGetter = selectNodeContents$1
            ? selectNodeContents
            : selectNode;
    ...
    const WindowBridge = (win) => {
        const elementFromPoint = (x, y) => {
            return SugarElement.fromPoint(
                SugarElement.fromDom(win.document),
                x,
                y,
            );
        };
                    ...
                    const update$1 = (attempts) => {
                        return () => {
                            ...
                            return navigation.fold(
                                ...
                                (_) => {
                                    return Optional.some(
                                        Response.create(Optional.none(), true),
                                    );
                                },
                            );
                        };
    ...
    const resizableRows = (warehouse, isResizable) =>
        bind$2(warehouse.all, (row, i) =>
            isResizable(row.element) ? [i] : [],
        );
    ...
    const refreshGrid = (warhouse, wire, table, rows, cols) => {
        ...
        const rowPositions =
            rows.length > 0 ? height.positions(rows, table) : [];
        ...
        const colPositions =
            cols.length > 0 ? width.positions(cols, table) : [];
        const resizableColBars =
            colPositions.length > 0
                ? resizableColumns(warhouse, isResizable)
                : [];
    ...
    const TableResizeHandler = (editor) => {
        ...
        const lazyResizingBehaviour = () =>
            isPreserveTableColumnResizing(editor)
                ? preserveTable()
                : resizeTable();
            ...
            if (isTable(targetElm)) {
                ...
                each$2(editor.dom.select(".mce-clonedresizable"), (clone) => {
                    editor.dom.addClass(
                        clone,
                        "mce-" +
                            getTableColumnResizingBehaviour(editor) +
                            "-columns",
                    );
                });
                ...
                if (
                    isNoneSizing(table) &&
                    startsWith(e.origin, barResizerPrefix)
                ) {
                    convertToPercentSize(table);
                }
                startW = e.width;
