...
(function () {
    ...
    const collectTextToBoundary = (dom, section, node, rootNode, forwards) => {
        ...
        const rootBlock =
            (_a = dom.getParent(rootNode, dom.isBlock)) !== null &&
            _a !== void 0
                ? _a
                : dom.getRoot();
    ...
    const fromNodes = (dom, nodes) =>
        bind(nodes, (node) => fromNode(dom, node));
    ...
    const mark = (matches, replacementNode) => {
        eachr(matches, (match, idx) => {
            eachr(match, (pos) => {
                const wrapper = SugarElement.fromDom(
                    replacementNode.cloneNode(false),
                );
    ...
    const open = (editor, currentSearchState) => {
        ...
        const getDialogSpec = (showNoMatchesAlertBanner, initialData) => ({
            ...
            onChange: (api, details) => {
                ...
                if (
                    details.name === "findtext" &&
                    currentSearchState.get().count > 0
                ) {
                    reset(api);
                }
            },
