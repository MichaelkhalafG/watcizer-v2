...
(function () {
                    ...
                    if (updateAll) {
                        switch (name) {
                            case "video":
                                ...
                                node.attr("src", null);
                                for (
                                    let index = numSources;
                                    index < 2;
                                    index++
                                ) {
                                    if (data[sources[index]]) {
                                        const source = new global$2(
                                            "source",
                                            1,
                                        );
                                        source.attr(
    ...
    const matchPattern = (url) => {
        const patterns = urlPatterns.filter((pattern) =>
    ...
    const showDialog = (editor) => {
        ...
        const handleUpdate = (api, sourceInput, prevData) => {
            ...
            const data =
                shouldInsertAsNewIframe(prevData, dialogData) &&
                hasDimensions(editor)
                    ? {
                          ...dialogData,
                          embed: "",
                      }
    ...
    const appendNodeContent = (editor, nodeName, previewNode, html) => {
        const newNode = Parser(editor.schema).parse(html, {
            context: nodeName,
        });
            ...
            serializer.addAttributeFilter("data-mce-object", (nodes, name) => {
                ...
                while (i--) {
                    ...
                    if (innerHtml) {
                        ...
                        each$1(fragment.children(), (child) =>
                            realElm.append(child),
                        );
                    }
