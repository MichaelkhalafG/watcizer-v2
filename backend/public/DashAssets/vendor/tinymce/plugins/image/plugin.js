...
(function () {
    ...
    const getImageSize = (url) =>
        new Promise((callback) => {
            ...
            img.onerror = () => {
                    ...
                    Promise.reject(
                        `Failed to get image dimensions for: ${url}`,
                    ),
                );
            };
    ...
    const blobToDataUri = (blob) =>
        new Promise((resolve, reject) => {
            ...
            reader.onerror = () => {
                var _a;
                reject(
                    (_a = reader.error) === null || _a === void 0
                        ? void 0
                        : _a.message,
                );
            };
    ...
    const getBorderStyle = (image) => {
        var _a;
        return (_a = image.style.borderStyle) !== null && _a !== void 0
            ? _a
            : "";
    };
    ...
    const getStyleValue = (normalizeCss, data) => {
        ...
        return normalizeCss(
            (_a = image.getAttribute("style")) !== null && _a !== void 0
                ? _a
                : "",
        );
    };
    ...
    const getSelectedImage = (editor) => {
        ...
        if (
            imgElm &&
            (imgElm.nodeName !== "IMG" || isPlaceholderImage(imgElm))
        ) {
            return null;
        }
        return imgElm;
    ...
    const sanitizer =
        ...
        (list) => {
            if (list) {
                return Optional.from(list).map((list) =>
                    sanitizeList(list, extractor),
                );
            } else {
                return Optional.none();
            }
    ...
    const submitHandler = (editor, info, helpers) => (api) => {
        ...
        const finalData = {
            ...data,
            style: getStyleValue(
                helpers.normalizeCss,
                toImageData(data, false),
            ),
        };
        editor.execCommand(
            "mceUpdateImage",
