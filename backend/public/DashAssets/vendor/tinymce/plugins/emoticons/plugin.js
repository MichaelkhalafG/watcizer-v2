...
(function () {
    ...
    const emojiMatches = (emoji, lowerCasePattern) =>
        ...
        exists(emoji.keywords, (k) =>
            contains(k.toLowerCase(), lowerCasePattern),
        );
    const emojisFrom = (list, pattern, maxResults) => {
        ...
        for (let i = 0; i < list.length; i++) {
            if (
                pattern.length === 0 ||
                emojiMatches(list[i], lowerCasePattern)
            ) {
                matches.push({
                    value: list[i].char,
                    text: list[i].title,
