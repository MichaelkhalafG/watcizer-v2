...
(function () {
    ...
    const outdentDlItem = (editor, item) => {
        if (isDescriptionDetail(item)) {
            ...
            parentElement(item).each((dl) =>
                splitList(editor, dl.dom, item.dom),
            );
        }
    };
    ...
    const getClosestEditingHost = (editor, elm) => {
        ...
        return parentTableCell.length > 0
            ? parentTableCell[0]
            : editor.getBody();
    };
    ...
    const isListInsideAnLiWithFirstAndLastNotListElement = (list) =>
        parent(list).exists(
            (parent) =>
                ...
                lastChild(parent).exists(
                    (lastChild) => !isListNode(lastChild.dom),
                ),
        );
    ...
    const getSelectedListRoots = (editor) => {
        ...
        return find(parentLists, (p) =>
            isListInsideAnLiWithFirstAndLastNotListElement(
                SugarElement.fromDom(p),
            ),
        ).fold(
            ...
            return [
                {
                    depth: depth + 1,
                    content:
                        (_a = item.dom.nodeValue) !== null && _a !== void 0
                            ? _a
                            : "",
                    dirty: false,
                    isSelected: false,
    ...
    const outdentedComposer = (editor, entries) => {
        ...
        return map(normalizedEntries, (entry) => {
            const content = !isEntryComment(entry)
                ...
                : fromElements([
                      SugarElement.fromHtml(`<!--${entry.content}-->`),
                  ]);
    ...
    const outdentListSelection = (editor) =>
        handleIndentation(editor, "Outdent");
    const flattenListSelection = (editor) =>
        handleIndentation(editor, "Flatten");
    ...
    const ancestor = (element, target) =>
        ancestor$1(element, curry(eq, target));
    ...
    const getEndPointNode = (editor, rng, start, root) => {
        ...
        const findBetterContainer = (container, forward) => {
            var _a;
            const walker = new global$5(
                container,
                findBlockAncestor(container),
            );
        ...
        if (start && isTextNode$1(container)) {
            if (isZwsp(container.textContent)) {
                container = findBetterContainer(container, false).getOr(
                    container,
                );
            } else {
                if (
        ...
        if (!start && isTextNode$1(container)) {
            if (isZwsp(container.textContent)) {
                container = findBetterContainer(container, true).getOr(
                    container,
                );
            } else {
                if (
    ...
    const getRootSearchStart = (editor, range) => {
        ...
        const startPoint = getEndPointNode(
            editor,
            range,
            true,
            editor.getBody(),
    ...
    const applyList = (editor, listName, detail) => {
        ...
        let listItemName = "LI";
        const root = getClosestListHost(
            editor,
            getRootSearchStart(editor, rng),
        );
        const dom = editor.dom;
    ...
    const isUnwrappable = (node) =>
        Optional.from(node)
            ...
            .exists(
                (el) => isEditable(el) && !contains$1(["details"], name(el)),
            );
    ...
    const backspaceDeleteFromListToListCaret = (editor, isForward) => {
        ...
        if (li) {
            ...
            const willMergeParentIntoChild =
                otherLi &&
                (isForward
                    ? dom.isChildOf(li, otherLi)
                    : dom.isChildOf(otherLi, li));
    ...
    const updateList = (editor, update) => {
        ...
        editor.undoManager.transact(() => {
            ...
            if (isObject(update.attrs)) {
                each(update.attrs, (v, k) =>
                    editor.dom.setAttrib(parentList, k, v),
                );
            }
        });
    };
