...
(function () {
    ...
    const isInSummary = (editor) => {
        ...
        return (
            isSummary(node) || Boolean(editor.dom.getParent(node, isSummary))
        );
    };
    const isAtDetailsStart = (editor) => {
        const rng = editor.selection.getRng();
        return (
            isDetails(rng.startContainer) &&
            rng.collapsed &&
            rng.startOffset === 0
        );
    };
    ...
    const getSelectedDetails = (editor) =>
        Optional.from(
            editor.dom.getParent(editor.selection.getNode(), isDetails),
        );
    ...
    const insertAccordion = (editor) => {
        ...
        const bodyText = editor.dom.encode(
            editor.translate("Accordion body..."),
        );
    ...
    const toggleAllAccordions = (editor, state) => {
        const accordions = Array.from(
            editor.getBody().querySelectorAll("details"),
        );
    ...
    const getAccordionChildren = (accordionNode) => {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (child.name === "summary" && isNullable(summaryNode)) {
                ...
                isAccordionBodyWrapperNode(child) &&
                isNullable(wrapperNode)
            ) {
                wrapperNode = child;
            } else {
    ...
    const onSetup = (editor) => (buttonApi) => {
        const onNodeChange = () =>
            buttonApi.setEnabled(isInsertAllowed(editor));
