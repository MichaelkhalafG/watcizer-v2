...
(function () {
    ...
    const siblings$2 = (element) => {
        const filterSelf = (elements) =>
            filter$1(elements, (x) => !eq(element, x));
    ...
    const bind = (name, callback, _win) =>
        get(_win)
            .map((visualViewport) => {
                ...
                return {
                    unbind: () =>
                        visualViewport.removeEventListener(name, handler),
                };
            })
    ...
    const toggleFullscreen = (editor, fullscreenState) => {
        ...
        if (!fullscreenInfo) {
            ...
            const newFullScreenInfo = {
                ...
                sinkCssPosition: sinkContainerS.map((elm) =>
                    get$2(elm, "position"),
                ),
            };
            if (isTouch) {
            ...
            lift2(
                sinkContainerS,
                fullscreenInfo.sinkCssPosition,
                (elm, val) => {
