...
(function () {
    ...
    const child$1 = (scope, selector) =>
        child$2(scope, (e) => is$2(e, selector));
    ...
    const children$2 = (scope, predicate) =>
        filter(children$3(scope), predicate);
    ...
    const removeLeading = (str, prefix) => {
        return startsWith(str, prefix)
            ? removeFromStart(str, prefix.length)
            : str;
    };
    ...
    const isValidSection = (parentName) =>
        contains(validSectionList, parentName);
        ...
        return lift2(cellOpt, rowsOpt, (cell, rows) =>
            filter(rows, (row) =>
                exists(
                    ...
                    (rowCell) =>
                        get$2(rowCell, selector) === "1" || eq(rowCell, cell),
                ),
            ),
        ).getOr([]);
    };
    ...
    const getAlignment = (formats, formatName, editor, elm) =>
        find(
            ...
            (name) =>
                !isUndefined(
                    editor.formatter.matchNode(elm, formatName + name),
                ),
        ).getOr("");
    ...
    const extractDataFromSettings = (editor, hasAdvTableTab) => {
        ...
        const extractAdvancedStyleData = () => ({
            ...
            backgroundcolor: rgbToHex(
                get$4(style, "background-color").getOr(""),
            ),
        });
        const defaultData = {
            height: "",
    ...
    const extractDataFromTableElement = (editor, elm, hasAdvTableTab) => {
        ...
        return {
            ...
            cellspacing:
                cellspacing !== null && cellspacing !== void 0
                    ? cellspacing
                    : "",
            cellpadding:
                cellpadding !== null && cellpadding !== void 0
                    ? cellpadding
                    : "",
    ...
    const getSelectionTargets = (editor) => {
        ...
        const getExtractedDetails = (targets) => {
            ...
            return tableOpt.map((table) => {
                ...
                return {
                    ...
                    unmergeable: onUnlockedUnmergable(
                        warehouse,
                        targets,
                    ).isSome(),
                    locked,
        ...
        const setupHandler = (handler) => {
            ...
            return () => {
                changeHandlers.set(
                    filter(changeHandlers.get(), (h) => h !== handler),
                );
            };
        };
        ...
        const onSetupPasteable = (getClipboardData) => (api) =>
            onSetup(
                api,
                (targets) =>
                    isCaption(targets.element) || getClipboardData().isNone(),
            );
        const onSetupPasteableColumn =
            (getClipboardData, lockedDisable) => (api) =>
                onSetup(
                    api,
                    (targets) =>
                        isCaption(targets.element) ||
