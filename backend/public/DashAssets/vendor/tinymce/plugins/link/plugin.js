...
(function () {
    ...
    const register$1 = (editor) => {
        ...
        registerOption("link_target_list", {
            processor: (value) =>
                isBoolean(value) || isArrayOf(value, isObject),
            default: true,
        });
    ...
    const extractFromAnchor = (editor, anchor) => {
        ...
        const rel = anchor.bind((anchorElm) =>
            nonEmptyAttr(dom, anchorElm, "rel"),
        );
    ...
    const handleSubmit = (editor, info) => (api) => {
        ...
        const getChangedValue = (key) =>
            Optional.from(data[key]).filter(
                (value) => !is(info.anchor[key], value),
            );
        const changedData = {
            href: data.url.value,
    ...
    const register = (editor) => {
        editor.addCommand("mceLink", (_ui, value) => {
            if (
                (value === null || value === void 0 ? void 0 : value.dialog) ===
                    true ||
                !useQuickLink(editor)
            ) {
                open$1(editor);
    ...
    const toggleUnlinkState = (editor) => (api) => {
        ...
        const updateEnabled = (parents) => {
            api.setEnabled(
                hasLinks$1(parents) && editor.selection.isEditable(),
            );
        };
    ...
    const setupContextMenu = (editor) => {
        ...
        editor.ui.registry.addContextMenu("link", {
            update: (element) => {
                ...
                return hasLinks(editor.dom.getParents(element, "a"))
                    ? inLink
                    : noLink;
            },
        });
