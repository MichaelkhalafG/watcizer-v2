...
class Animation {
    constructor(cfg, target, prop, to) {
        ...
        this._easing =
            helpers_segment.effects[cfg.easing] ||
            helpers_segment.effects.linear;
...
class Animations {
    ...
    configure(config) {
        ...
        const animationOptions = Object.keys(
            helpers_segment.defaults.animation,
        );
...
function applyStack(stack, value, dsIndex, options = {}) {
    ...
    for (i = 0, ilen = keys.length; i < ilen; ++i) {
        ...
        if (
            ...
            (singleMode ||
                value === 0 ||
                helpers_segment.sign(value) ===
                    helpers_segment.sign(otherValue))
        ) {
            value += otherValue;
        }
    }
...
class DatasetController {
    ...
    _resolveElementOptions(elementType, mode = "default", index) {
        ...
        const sharing =
            this.enableOptionSharing && helpers_segment.defined(index);
        if (cached) {
        ...
        const names = Object.keys(
            helpers_segment.defaults.elements[elementType],
        );
    ...
    _calculateBarValuePixels(index) {
        ...
        if (floating) {
            ...
            if (
                value !== 0 &&
                helpers_segment.sign(value) !==
                    helpers_segment.sign(custom.barEnd)
            ) {
                start = 0;
            }
            start += value;
...
class PolarAreaController extends DatasetController {
    ...
    updateElements(arcs, start, count, mode) {
        ...
        const datasetStartAngle =
            scale.getIndexAngle(0) - 0.5 * helpers_segment.PI;
...
function initCanvas(canvas, aspectRatio) {
    ...
    if (isNullOrEmpty(renderHeight)) {
        if (canvas.style.height === "") {
            ...
            const displayHeight = helpers_segment.readUsedSize(
                canvas,
                "height",
            );
...
class DomPlatform extends BasePlatform {
    ...
    getMaximumSize(canvas, width, height, aspectRatio) {
        return helpers_segment.getMaximumSize(
            canvas,
            width,
            height,
            aspectRatio,
...
class Element {
    ...
    hasValue() {
        return (
            helpers_segment.isNumber(this.x) && helpers_segment.isNumber(this.y)
        );
    }
...
function getTitleHeight(options, fallback) {
    ...
    const lines = helpers_segment.isArray(options.text)
        ? options.text.length
        : 1;
...
class Scale extends Element {
    ...
    fit() {
        ...
        if (display) {
            ...
            if (tickOpts.display && this.ticks.length) {
                ...
                const angleRadians = helpers_segment.toRadians(
                    this.labelRotation,
                );
    ...
    drawBorder() {
        ...
        if (this.isHorizontal()) {
            ...
            y1 =
                helpers_segment._alignPixel(chart, this.top, axisWidth) -
                axisWidth / 2;
            y2 =
    ...
    _layers() {
        ...
        const bz = helpers_segment.valueOrDefault(
...
class Config {
    ...
    getOptionScopes(mainScope, keyLists, resetCache) {
        ...
        keyLists.forEach((keys) => {
            ...
            keys.forEach((key) =>
                addIfFound(scopes, helpers_segment.defaults, key),
            );
            keys.forEach((key) =>
    ...
    resolveNamedOptions(scopes, names, context, prefixes = [""]) {
        ...
        if (needContext(resolver, names)) {
            ...
            options = helpers_segment._attachContext(
                resolver,
                context,
                subResolver,
            );
...
class Chart {
    ...
    bindUserEvents() {
        ...
        helpers_segment.each(this.options.events, (type) =>
            _add(type, listener),
        );
    }
...
class ArcElement extends Element {
    ...
    draw(ctx) {
        ...
        const fix =
            1 - Math.sin(Math.min(helpers_segment.PI, circumference || 0));
...
function _decodeFill(line, index, count) {
    ...
    if (
        helpers_segment.isNumberFinite(target) &&
...
class Legend extends Element {
    ...
    _draw() {
        ...
        const drawLegendBox = function (x, y, legendItem) {
            ...
            ctx.save();
            const lineWidth = helpers_segment.valueOrDefault(
            ...
            ctx.setLineDash(
                helpers_segment.valueOrDefault(legendItem.lineDash, []),
            );
            if (labelOpts.usePointStyle) {
                const drawOptions = {
    ...
    drawTitle() {
        ...
        const x = helpers_segment._alignStartEnd(
            position,
            left,
            left + maxWidth,
        );
        ...
        labels: {
            ...
            generateLabels(chart) {
                ...
                return chart._getSortedDatasetMetas().map((meta) => {
                    ...
                    const borderWidth = helpers_segment.toPadding(
                        style.borderWidth,
                    );
                    return {
...
class Title extends Element {
    ...
    update(maxWidth, maxHeight) {
        ...
        const lineCount = helpers_segment.isArray(opts.text)
            ? opts.text.length
            : 1;
    ...
    nearest(items, eventPosition) {
        ...
        for (i = 0, len = items.length; i < len; ++i) {
            ...
            if (el && el.hasValue()) {
                ...
                const d = helpers_segment.distanceBetweenPoints(
                    eventPosition,
                    center,
                );
                if (d < minDistance) {
...
function generateTicks$1(generationOptions, dataRange) {
    ...
    if (numSpaces > maxSpaces) {
        spacing =
            helpers_segment.niceNum((numSpaces * spacing) / maxSpaces / unit) *
            unit;
    }
...
class LogarithmicScale extends Scale {
    ...
    parse(raw, index) {
        ...
        return helpers_segment.isNumberFinite(value) && value > 0
            ? value
            : null;
    }
    determineDataLimits() {
        const { min, max } = this.getMinMax(true);
        this.min = helpers_segment.isNumberFinite(min)
            ? Math.max(0, min)
            : null;
        this.max = helpers_segment.isNumberFinite(max)
            ? Math.max(0, max)
            : null;
    ...
    getPixelForValue(value) {
        ...
        return this.getPixelForDecimal(
            value === this.min
                ? 0
                : (helpers_segment.log10(value) - this._startValue) /
                      this._valueRange,
        );
    }
    getValueForPixel(pixel) {
    ...
    drawLabels() {
        ...
        this.ticks.forEach((tick, index) => {
            ...
            if (optsAtIndex.showLabelBackdrop) {
                ...
                const padding = helpers_segment.toPadding(
                    optsAtIndex.backdropPadding,
                );
                ctx.fillRect(
...
class TimeScale extends Scale {
    ...
    _generate() {
        ...
        const stepSize = helpers_segment.valueOrDefault(
            options.ticks.stepSize,
            1,
        );
        const weekday = minor === "week" ? timeOpts.isoWeekday : false;
        const hasWeekday =
            helpers_segment.isNumber(weekday) || weekday === true;
        const ticks = {};
        let first = min;
        let time, count;
        if (hasWeekday) {
    ...
    _tickFormatFunction(time, index, ticks, format) {
        ...
        if (formatter) {
            return helpers_segment.callback(
                formatter,
                [time, index, ticks],
                this,
            );
