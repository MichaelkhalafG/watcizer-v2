...
- [ ] **P1-C2 (catalog/meta lookup migration) — HELD.** `catalog/meta` can't replace the 11 legacy lookup tables that `transformProductData` consumes: it omits 7 of them (`categoryTypes, materials, shapes, sizeTypes, displayTypes, closureTypes, movementTypes`) and uses a flat `{id,name_en,name_ar}` shape instead of the raw `{translations:[…]}` the transform expects. Either keep it additive (powering only the new filter UIs) or expand `catalog/meta` to carry those 7 tables in raw shape before retiring the legacy fetches. — discovered during P1-C2
- [ ] **P1-C3 (Listing server-side filtering) — LATER.** Listing filter state is multi-select arrays (`{categories[],brands[],subTypes[],price[min,max]}`, set by `SideBar`) and `categories` holds **categoryType** ids (Watches/Fashion), but `/api/products` takes single-value params and filters `main_category_id` (no categoryType filter). To migrate: extend `/products` to accept `category_type_id` + comma-list multi-values, OR redesign the filters to single-select + `main_category_id`. `Listing.jsx:41-138`, `SideBar.jsx:65-82`. — discovered during P1-C3
- [ ] **P1-C4 frontend (ProductDisplay → `/products/by-name`) — DEFERRED.** Backend `GET /api/products/by-name/{name}` is built & verified. The PDP data-source swap is blocked by `ProductResource` shape gaps vs the `transformProductData` shape `ProductDisplay` renders: missing size-type names, `band_closure`, `dial_display_type`, `country`, `stone`, `band_width`, watch dimensions, `category_type`; `brand`/`grade`/`sub_type` returned as objects (PD reads strings); `dial_color`/`band_color` lack `color_value`; `glass_material`/`case_material` vs PD's `dial_glass_material`/`dial_case_material`; `features`/`gender` are object arrays (PD `.join()`s strings). Expand `ProductResource` to full parity (or update PD field reads) before swapping. — discovered during P1-C4
-   P 0 - 7 :   h s _ c o d e   i n   P r o d u c t   \   i s   n o t   a n   a c t u a l   c o l u m n      h a r m l e s s   b u t   s h o u l d   b e   r e m o v e d   f r o m   \   i n   P 3 - 4   d e a d   c o d e   c l e a n u p   p a s s .  
 