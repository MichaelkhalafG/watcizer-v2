...
// SSR prefetch reuses the exact same fetch + return shape (queryKey ['tables']).
export const tablesQueryFn = (client = http) => async () => {
  const { data } = await client.get('catalog/meta')
  return data.tables
}
