...
@php($copyright = $copyright ?? config('notifications.brand.copyright'))
{{-- ^ the ONE line changed when these templates were ported from backend/ (2026-09-13):
     core has no `config/watchizer.php`, so the fallback reads core's own key. Every other
     byte of all six files is identical to the legacy originals — verified by md5. --}}
